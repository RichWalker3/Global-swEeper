...
Rebase or merge `feat/sfcc-wa-parity` onto the unified `prod` baseline before any PROD release.

**Pending on `feat/sfcc-wa-parity` (uncommitted local work — include in next release notes / CHANGELOG Unreleased → version bump):**

| Feature | Status | Files | Release note |
|---------|--------|-------|----------------|
| BRD Workspace Phase dropdown → Jira `customfield_21069` | Implemented locally, not on `main` | `src/web/public/index.html`, `src/brd/jira.ts`, `src/brd/types.ts`, `src/brd/composer.ts` | See CHANGELOG **Unreleased** |
| Pre-order detector fix (notify-me ≠ pre-order) | Todo only | `src/scraper/catalogDetector.ts` | — |
| Returns provider detection (Loop, ReturnGO, etc.) | Todo only | `src/scraper/policyExtractor.ts`, `detectors.ts`, `scraper.ts` | — |

When cutting the release that merges this branch: move items from `CHANGELOG.md` **Unreleased** into the versioned section; mark done items in `docs/SWEEP_TODO.md`.
