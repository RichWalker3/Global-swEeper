...
**Want one prompt that does everything?** Use **`SETUP_PROMPT.txt`** at the repo root — new folder → file in folder → open in Cursor → **`@SETUP_PROMPT.txt`** **Read and run the setup prompt.** See **[ONE_SHOT_SETUP_PROMPT.md](./ONE_SHOT_SETUP_PROMPT.md)**.
